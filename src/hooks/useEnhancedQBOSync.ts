...
import { enhancedQboService } from '@/services/qbo/EnhancedQBOService';
import { supabase } from '@/integrations/supabase/client';
import { toast } from "@/hooks/use-toast";
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { v4 as uuidv4 } from 'uuid';

// Enhanced hook for QBO Sync operations with resilience patterns
export const useEnhancedQBOSync = (organizationId?: string) => {
  const { user } = useAuth();
  ...
  // Enhanced mutation to process pending operations with improved resilience
  const processPendingMutation = useMutation({
    mutationFn: async (options?: { entityType?: string; batchSize?: number }) => {
      // Use more sophisticated batching and concurrency control
      const batchSize = options?.batchSize || 10;
      const entityType = options?.entityType;
      
      // Get pending operations
      const pendingOps = entityType 
        ? await qboService.getPendingOperations(entityType, batchSize)
        : await qboService.getPendingOperations(undefined, batchSize);
      
      if (!pendingOps.length) {
        return { processed: 0, success: 0, failed: 0 };
      }
      
      // Use the enhanced service's batch processing
      return enhancedQboService.processBatch(pendingOps, 3); // Process with concurrency of 3
    },
    onSuccess: (result) => {
      if (result.processed > 0) {
        toast({
          title: "Enhanced sync progress",
